...
class Game
  ...
  def process_raw_boards
    RAW_BOARDS.each_slice(@board_count) do |board_slice|
      board_slice.each_slice(@row_count) do |row_slice|
        row_slice.each_with_index do |row, index|
          row_slice[index] = row.split
        end
        board = []
        board += row_slice
        @board_data << board
  ...
  def populate_boards(board_data)
    boards = []
  ...
  def any_winners?
    @boards.any? &:is_a_winner?
  end
  def all_winners?
    @boards.all? &:is_a_winner?
  end
  def play
    call = draw
    stamp(call)
  ...
  def summarize(call)
    puts "Last call was #{call}."
    puts "Sum of unmarked squares:"
    puts sum = @winner_sum
    binding.pry
    puts "Sum × last call:"
...
class Board
  ...
  def stamp(snum)
    @rows.each do |row|
      row.map! { |square| square == snum ? MARK : square }
    end
  end
  def load_cols
    @cols  = @rows.transpose
  ...
  def load_diags
    diags = []
    diags << find_diag(@rows)
    diags << find_diag(@cols)
  end
  def find_diag(lines)
    diag = []
  ...
  def type_wins?(type)
    type.any? { |line| line_wins?(line) }
  end
  def line_wins?(line)
    line.all? { |num| num == MARK }
  end
  def sum_unmarked
    total = 0
    @rows.each do |row|
...
# gm = Game.new(@board_data)
# until gm.any_winners?
#   gm.play
# end
# binding.pry
